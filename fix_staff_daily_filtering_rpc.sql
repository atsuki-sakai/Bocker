...
-- 既存の月次集計関数をバックアップ用に残し、新しい日次対応関数を作成
DROP FUNCTION IF EXISTS get_daily_staff_sales(TEXT, TEXT, DATE, DATE, TEXT[]);
...
BEGIN
    ...
    WHERE r.tenant_id = p_tenant_id
      ...
      AND r.business_date >= p_date_from
      AND r.business_date <= p_date_to
...
COMMENT ON FUNCTION get_daily_staff_sales IS '日次レベルでの正確な期間絞り込み対応スタッフ別売上集計関数';
...
- **移行対象**: 完了済み予約（24時間以上経過）とキャンセル済み予約（7日以上経過）
- **実行スケジュール**: 毎日午前2時（JST）に自動実行
- **処理方式**: バッチ処理による効率的な移行（500件/バッチ）
- **品質保証**: エラーハンドリングと移行ログの記録
...
**関連ドキュメント**:
- [詳細実装計画](./implementation-plan.md)
- [実行手順書](./execution-guide.md)
- [テスト計画書](./test-plan.md)
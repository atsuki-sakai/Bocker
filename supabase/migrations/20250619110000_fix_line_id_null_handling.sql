...
BEGIN
    ...
    -- line_idとline_user_nameは空文字列の場合は既存の値を保持（NULLの場合はNULLのまま）
    UPDATE public.customer SET
        ...
        phone = p_phone,
        line_id = CASE 
            WHEN p_line_id = '' OR p_line_id IS NULL THEN existing_line_id  -- NULLの場合はNULLのまま
            ELSE p_line_id
        END,
        line_user_name = CASE 
            WHEN p_line_user_name = '' OR p_line_user_name IS NULL THEN existing_line_user_name  -- NULLの場合はNULLのまま

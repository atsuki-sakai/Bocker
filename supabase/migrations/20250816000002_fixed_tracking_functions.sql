-- Migration: 修正版トラッキング機能のSupabase完全移行
-- 目的: 集計ロジックの問題を修正したバージョン
-- 作成日: 2025-08-16
-- 修正点: Unix timestamp計算のミリ秒対応、コンバージョン計算の安全性向上
...
BEGIN
    ...
    -- 修正: Unix timestampの範囲計算（ミリ秒対応）
    target_date_start := EXTRACT(EPOCH FROM target_date::timestamp) * 1000;
...
-- 関数にコメントを追加
COMMENT ON FUNCTION aggregate_daily_tracking_data IS '日次トラッキングデータの集計処理。修正版: ミリ秒対応とコンバージョン計算の修正';

-- ===============================
-- 3. データクリーンアップ関数の作成
-- ===============================

-- 古いトラッキングイベントの自動削除関数（修正版）
...
BEGIN
    ...
    -- 修正: ミリ秒対応
    cutoff_timestamp := EXTRACT(EPOCH FROM (CURRENT_DATE - (retention_days || ' days')::INTERVAL)) * 1000;
...
-- 既存のcronジョブを削除（重複防止）
SELECT cron.unschedule('daily-tracking-aggregation') WHERE EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'daily-tracking-aggregation'
);
SELECT cron.unschedule('weekly-tracking-cleanup') WHERE EXISTS (
    SELECT 1 FROM cron.job WHERE jobname = 'weekly-tracking-cleanup'
);
...
GRANT EXECUTE ON FUNCTION backfill_tracking_summaries TO service_role;
...
RAISE NOTICE 'Migration completed: Fixed tracking functionality has been applied to Supabase';
-- 実用的パーティション対応売上集計システム
-- 主キー制約とUPSERT動作を両立する設計
...
DROP FUNCTION IF EXISTS increment_sales_with_guard_v2 CASCADE;
...
-- 1. 日別売上集計テーブル（business_dateでパーティション）
...
-- 2. スタッフ別売上集計テーブル（月次summary_monthでパーティション）
CREATE TABLE staff_sales_summary (
  tenant_id text NOT NULL,
  org_id    text NOT NULL,
  staff_id  text NOT NULL,
  summary_month date NOT NULL, -- 集計月（YYYY-MM-01形式）
  staff_name text,
  total_amount  numeric(12,2) NOT NULL DEFAULT 0,
  booking_count int           NOT NULL DEFAULT 0,
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (tenant_id, org_id, staff_id, summary_month)
) PARTITION BY RANGE (summary_month);

-- 3. メニュー別売上集計テーブル（月次summary_monthでパーティション）
CREATE TABLE menu_sales_summary (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  PRIMARY KEY (tenant_id, org_id, menu_id, summary_month)
) PARTITION BY RANGE (summary_month);
...
RETURNS TRIGGER AS $$
DECLARE
  start_date date := NEW.summary_month;
  end_date date := start_date + interval '1 month';
...
BEGIN
  BEGIN
    ...
    EXECUTE format('
      CREATE INDEX IF NOT EXISTS %I ON %I (summary_month)',
      part_name || '_date_idx', part_name);
  EXCEPTION WHEN OTHERS THEN
...
BEGIN
  ...
  FOR i IN -6..6 LOOP
    ...
    part_name := 'daily_sales_summary_' || to_char(target_month, 'YYYYMM');
  ...
  FOR i IN -6..6 LOOP
    target_month := date_trunc('month', CURRENT_DATE) + (i || ' months')::interval;
    part_name := 'staff_sales_summary_' || to_char(target_month, 'YYYYMM');
    start_date := target_month;
    end_date := start_date + interval '1 month';
  ...
  FOR i IN -6..6 LOOP
    target_month := date_trunc('month', CURRENT_DATE) + (i || ' months')::interval;
    part_name := 'menu_sales_summary_' || to_char(target_month, 'YYYYMM');
    start_date := target_month;
    end_date := start_date + interval '1 month';
...
COMMENT ON TABLE staff_sales_summary IS 'リアルタイムスタッフ別売上集計テーブル（月次パーティション・summary_month基準）';
COMMENT ON TABLE menu_sales_summary IS 'リアルタイムメニュー別売上集計テーブル（月次パーティション・summary_month基準）';
COMMENT ON TABLE sales_aggregation_log IS '売上集計処理ログ（重複防止用）';

COMMENT ON FUNCTION increment_sales_with_guard_v2 IS '実用的パーティション対応統合売上集計更新関数（重複防止付き）';

...
test.describe('ホームページ', () => {
  ...
  test('多言語対応（タイ語）', async ({ page }) => {
    await page.goto('/th')
    await expect(page).toHaveURL(/\/th/)
    await expect(page.locator('main')).toBeVisible()
    await expect(page.locator('[lang="th-TH"]')).toBeVisible()
  })
  ...
  test('Clerkログインフローが正常に動作する', async ({ page }) => {
    ...
    if (loginButton) {
      ...
      await page.waitForSelector(
        'input[name="identifier"], input[type="email"], input[data-testid="identifier-field"]',
        { timeout: 10000 }
      )
      ...
      const continueButton = page
        .getByRole('button', { name: /continue|続行|次へ|Submit/i })
        .first()
      ...
      await page.waitForSelector(
        'input[name="password"], input[type="password"], input[data-testid="password-field"]',
        { timeout: 10000 }
      )
      ...
      const signInButton = page
        .getByRole('button', { name: /sign in|ログイン|サインイン|Submit/i })
        .first()

...
export function handleConvexApiError(message: string, code: ERROR_CODES, error?: unknown, context: Record<string, any> = {}) {
  console.error(
    code,
    message,
    { ...context },
...
export const removeEmptyFields = <T extends Record<string, unknown>>(object: T): Partial<T> => {
  ...
  if (Object.keys(result).length === 0) {
    throw new ConvexError({message: "更新するデータがありません", code: ERROR_CODES.INVALID_ARGUMENT});
  }
  return result;
};
...
export async function trashRecord<T extends keyof DataModel>(ctx: MutationCtx, id: Id<T>) {
  ...
  if (!record || (record as any).isArchive) {
    ...
    throw new ConvexError({message: "指定されたレコードが存在しないか、アーカイブされています", code: ERROR_CODES.NOT_FOUND});
  }
  // eslint-disable-next-line

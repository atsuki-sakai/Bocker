...
vi.mock('@/lib/types', () => ({
  ROLE_LEVEL: { admin: 4, owner: 3, manager: 2, staff: 1 },
  PLAN_LEVEL: { PRO: 3, LITE: 2, MICRO: 1, UNKNOWN: 0 },
}))
...
class MockFileReader {
  ...
  onerror: ((error: any) => void) | null = null // eslint-disable-line @typescript-eslint/no-explicit-any
...
describe('Utils Library', () => {
  ...
  describe('normalizeSubscriptionStatus function', () => {
    ...
    it('未知のステータスはそのまま返し警告を出力する', () => {
      ...
      const subscription = { id: 'sub_123', status: 'unknown_status' } as any // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  describe('sanitizeFileName function', () => {
    ...
    it('非文字列入力でTypeErrorをスローする', () => {
      expect(() => sanitizeFileName(null as any)).toThrow(TypeError) // eslint-disable-line @typescript-eslint/no-explicit-any
    })
  })
  ...
  describe('Cookie functions', () => {
    let mockDocument: any // eslint-disable-line @typescript-eslint/no-explicit-any

...
export default function ReservationNotificationList() {
    ...
    const notifications = useQuery(api.reservation.notification.query.list, tenantId && orgId ? {
        tenant_id: tenantId,
        org_id: orgId,
    } : "skip");
    ...
    const handleKillAllNotifications = () => {
        try {
            killAllNotifications({
                ...
                reservation_ids: notifications?.map((notification) => notification.reservation_id as Id<"reservation">) || [],
            });
        ...
        <div className="relative h-full w-full">
            {
                ...
                <div className="absolute -top-3 right-0 flex items-center justify-center gap-2 z-10">
                    ...
                    <AccordionItem value="notifications" >
                        <AccordionTrigger className="py-2 px-3 rounded-lg transition-colors hover:no-underline">
                                ...
                                <div className="flex flex-col md:flex-row items-start md:items-center md:gap-2 w-full">
                                    ...
                                    {notifications && notifications.length > 0 && (
                                        <span className="px-2 py-0.5 rounded-full text-xs">
                                           <strong className="text-base text-accent-2"> {notifications.length}</strong> 件の新規予約
                                        </span>
                                    )}
                                </div>
                                
                                    <Button className="mr-5" variant="destructive" size="sm" onClick={() => handleKillAllNotifications()}>
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col gap-1 w-full text-nowrap">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {format(notification.start_time_unix, 'yyyy年MM月dd日 HH:mm')} ~ {format(notification.end_time_unix, 'HH:mm')}</span>
                                            </div>

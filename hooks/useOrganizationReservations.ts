...
/**
 * ConvexのIDかSupabaseのIDかを判定してデータを取得するカスタムフック
 * @param reservationId - ConvexのIDまたはSupabaseのID
 * @returns 予約データ、ローディング状態、エラー状態
 */
export function useReservationData(reservationId: string) {
  const [supabaseData, setSupabaseData] = useState<IntegratedReservation | null>(null)
  const [supabaseLoading, setSupabaseLoading] = useState(false)
  const [supabaseError, setSupabaseError] = useState<string | null>(null)

  const reservationRepo = useMemo(() => new ReservationRepository(), [])

  // ConvexのIDかどうかを判定（ConvexのIDは特定の形式: 32文字のa-z0-9）
  const isConvexId = reservationId.match(/^[a-z0-9]{32}$/)

  // Convexからのデータ取得
  const convexData = useQuery(
    api.reservation.query.getWithDetailById,
    isConvexId ? { id: reservationId as Id<'reservation'> } : 'skip'
  )

  // Supabaseからのデータ取得
  useEffect(() => {
    async function fetchSupabaseData() {
      if (isConvexId || !reservationId) return

      setSupabaseLoading(true)
      setSupabaseError(null)

      try {
        const result = await reservationRepo.findById(reservationId)

        if (!result) {
          setSupabaseError('予約が見つかりませんでした')
          setSupabaseData(null)
        } else {
          // Supabaseデータを統合型に変換
          const converted: IntegratedReservation = {
            id: result.reservation.uid,
            source: 'supabase',
            tenantId: result.reservation.tenant_id,
            orgId: result.reservation.org_id,
            customerUid: result.reservation.customer_uid || '',
            staffId: result.reservation.staff_id,
            customerName: result.reservation.customer_name,
            staffName: result.reservation.staff_name,
            ...
            createdAt: new Date(result.reservation.created_at),
            detail: result.reservationDetail
              ? {
                  menus: result.reservationDetail.menus
                    ? parseReservationMenus(result.reservationDetail.menus)
                    : undefined,
                  options: result.reservationDetail.options
                    ? parseReservationOptions(result.reservationDetail.options)
                    : undefined,
                  totalPrice: result.reservationDetail.total_price || undefined,
                  paymentMethod: result.reservationDetail.payment_method as PaymentMethod,
                  couponId: result.reservationDetail.coupon_id || undefined,
                  couponDiscount: result.reservationDetail.coupon_discount || undefined,
                  usePoints: result.reservationDetail.use_points || undefined,
                  notes: result.reservationDetail.notes || undefined,
                }
              : undefined,
            supabaseData: {
          ...
          setSupabaseData(converted)
        }
      } catch (error) {
        console.error('Failed to fetch reservation from Supabase:', error)
        setSupabaseError('予約データの取得に失敗しました')
        setSupabaseData(null)
      } finally {
    ...
    fetchSupabaseData()
  }, [reservationId, isConvexId, reservationRepo])

  // 結果の統合
  if (isConvexId) {
    // Convexデータを統合型に変換
    const convexIntegrated: IntegratedReservation | null = convexData
      ? {
          id: convexData.reservation._id,
          source: 'convex',
          tenantId: convexData.reservation.tenant_id,
          orgId: convexData.reservation.org_id,
          customerUid: convexData.reservation.customer_uid || '',
          staffId: convexData.reservation.staff_id,
          customerName: convexData.reservation.customer_name,
          staffName: convexData.reservation.staff_name,
          ...
          createdAt: new Date(convexData.reservation._creationTime),
          detail: convexData.reservationDetail
            ? {
                menus: convexData.reservationDetail.menus,
                options: convexData.reservationDetail.options,
                totalPrice: convexData.reservationDetail.total_price,
                extraCharge: convexData.reservationDetail.extra_charge,
                paymentMethod: convexData.reservationDetail.payment_method,
                couponId: convexData.reservationDetail.coupon_id,
                couponDiscount: convexData.reservationDetail.coupon_discount,
                usePoints: convexData.reservationDetail.use_points,
                notes: convexData.reservationDetail.notes,
              }
            : undefined,
          convexData: convexData.reservation,
        }
      : null

    return {
      data: convexIntegrated,
...
export function useOrganizationReservations({
  ...
  const fetchStats = useCallback(async () => {
    ...
    console.log('[useOrganizationReservations] Fetching stats with params:', {
      tenantId,
      orgId,
      startDate,
      endDate,
  ...
  const fetchSupabaseReservations = useCallback(
    async (page: number, reset: boolean = false) => {
      ...
      try {
            ...
            detail: item.detail
              ? {
                  ...
                  options: item.detail.options
                    ? parseReservationOptions(item.detail.options)
                    : undefined,
        ...
        console.log('[useOrganizationReservations] Supabase data fetched successfully:', {
  ...
  const integratedReservations = useMemo(() => {
    console.log('[useOrganizationReservations] Data integration:', {
      ...
      convexSample: convexReservations
        .slice(0, 2)
        .map((r) => ({ id: r.id, date: r.date, status: r.status, source: r.source })),
      supabaseSample: supabaseReservations
        .slice(0, 2)
        .map((r) => ({ id: r.id, date: r.date, status: r.status, source: r.source })),
    })
  ...
  const isLoading = useMemo(() => {
    ...
    status,
    useDataRangeQuery,
    convexReservations.length,
    supabaseReservations.length,
    integratedReservations.length,
    canFetchConvex,
    skipConvexForHistoricalData,
  ])
  ...
  const calculatedStats = useMemo(() => {
    ...
    const convexActiveCount = convexReservations.filter(
      (r) => r.status === 'confirmed' || r.status === 'pending'
    ).length

    const convexUpcomingCount = convexReservations.filter(
      (r) => (r.status === 'confirmed' || r.status === 'pending') && r.startTimeUnix > Date.now()
    ).length

...
|------|-----|------|
| 店舗数 | 3,000 店 | - |
| スタッフ数 | 5 名/店 | プロフィール画像 1 枚 (170 KB) |
| メニュー数 | 15 件/店（最大 20 件）| 画像 3 枚/メニュー (175 KB/枚) |
| 物販 SKU | 10 件/店 | 画像 1 枚/SKU (175 KB) |
| 予約件数 | 25–40 件/店/日 (平均 32.5) | - |
| 予約更新率 | 5 % | 20 件に 1 件更新 |
| 新規顧客 | 30–50 件/店/日 (平均 40) | - |
| カルテ作成率 | 100 % | 予約 1 件 = カルテ 1 件 |
| カルテ画像 | 4 枚/カルテ | 175 KB/枚 |
| 理想ヘア画像 | 1 枚/予約 | 175 KB/枚 |
| 会計明細 | 20 行/予約 | 1 レコード ≒ 1 KB |
| ポイント・クーポン | 付与 1 件 / 利用 2–3 件/予約 | - |
| 通知 | 2 通/予約 (顧客) | スタッフ通知追加予定 |
| 音声メモ | 非保存 | STTテキストのみ (0.5 KB/件) |
| ログ保持期間 | 30 日 | 以降ロールアップ |

### データ生成量（1店舗/日）

| 区分 | 件数/日 | 合計 |
|------|---------|------|
| 画像 (カルテ) | 4 × 175 KB × 25–40 | 17.1–27.3 MB |
| 画像 (ヘア参考) | 1 × 175 KB × 25–40 | 4.3–7.0 MB |
| テキスト+メタデータ | 予約・会計など | 0.71–1.11 MB |
| **合計 (画像)** | — | 21.4–34.3 MB |
| **合計 (全体)** | — | 22.1–35.4 MB |
...
| 区分 | 年間増分 | 備考 |
|------|----------|------|
| 画像 | 22.8–36.6 TB | 21.4–34.3 MB × 3000 × 365 |
| テキスト | ~1.2 TB | 1.1 MB × 3000 × 365 |
| **年間合計** | 24–38 TB | - |

### Google Cloud Storage (GCS) ストレージ方針

#### バケット構成

```
└─ gs://bcker-prod-images (Single bucket, asia-northeast1)
    • 画像・サムネイルとも同一バケット内に保存
    • オブジェクト名: yyyy/mm/dd/<uuid>.webp
```

#### Lifecycle ルール

30日経過で Coldline、365日経過で Archive に自動クラス変更：
...
適用コマンド：
```bash
gsutil lifecycle set lifecycle.json gs://bcker-prod-images
```
...
| フェーズ | 容量 (年間平均) | 単価* | 年額 |
|----------|-----------------|-------|------|
| Standard (直近 30 日) | 3.2 TB | $0.023/GB | ≈ $885 |
| Coldline (31-365 日) | 34.8 TB | $0.006/GB | ≈ $2,090 |
| Archive (>1 年) | → ローテーション | $0.0025/GB | +α |
| **小計** | — | — | ≈ $3,000/年（約 45 万円）|
...
| 項目 | 内容 |
|------|------|
| 画像圧縮 | アップロード時に WebP/AVIF (品質 80%) 強制変換 → ~30% 削減 |
| サムネイル | Cloud Functions で 256px を自動生成し同バケット保存 |
| Cloud CDN | Cache-Control: public, max-age=2592000 で 30 日キャッシュ |
| アラート | Cloud Billing Budget：前月比 20% 超過で Slack 通知 |

### 運用 TODO

1. 月次でバケット使用量を BigQuery にエクスポートし、増分を可視化する
2. スタッフ通知（メール/LINE）機能追加時に通知レコード数を再試算する
3. 画像バージョニングは OFF。変更頻度が非常に低いため冗長保管を避ける

### 取得課金早見表

| 取得課金 | Coldline | Archive |
|----------|----------|---------|
| Retrieval Fee | $0.02/GB | $0.05/GB |
| Class-B Ops | $0.004/10k GET | 同左 |

**Tips**: 大量復元時は一時的に Standard にコピーして作業→30日後に自動降格でコスト最適化
...
ビジネスサイドの事はPRODUCT.mdを参照してください
運用コスト関連はPRODUCT_COST.mdを参照してください

...
class GoogleStorageService {
  ...
  private formatErrorDetails(error: unknown): Record<string, unknown> {
    ...
    if (error instanceof Error) {
      ...
      try {
        ...
        errorDetails.originalValue = String(e)
      }
    }
    return errorDetails
  }
  ...
  private async validateImageSafety(
    ...
    if (imageBuffer.length > IMAGE_LIMITS.MAX_FILE_SIZE) {
      throw new SystemError('画像ファイルサイズが制限を超えています。', {
    ...
    try {
      ...
      if (width > IMAGE_LIMITS.MAX_DIMENSION || height > IMAGE_LIMITS.MAX_DIMENSION) {
        throw new SystemError('画像の解像度が制限を超えています。', {
          statusCode: ERROR_STATUS_CODE.UNPROCESSABLE_ENTITY,
          severity: ERROR_SEVERITY.ERROR,
          callFunc: 'GoogleStorageService.validateImageSafety',
      ...
      if (estimatedMemoryUsage > IMAGE_LIMITS.MAX_MEMORY_USAGE) {
        throw new SystemError('画像処理に必要なメモリが制限を超えています。', {
          statusCode: ERROR_STATUS_CODE.UNPROCESSABLE_ENTITY,
          severity: ERROR_SEVERITY.ERROR,
          callFunc: 'GoogleStorageService.validateImageSafety',
      ...
      throw new SystemError('画像の形式が不正または破損している可能性があります。', {
        statusCode: ERROR_STATUS_CODE.UNPROCESSABLE_ENTITY,
        severity: ERROR_SEVERITY.ERROR,
        callFunc: 'GoogleStorageService.validateImageSafety',
  ...
  private async compressImage(
    ...
    if (inputBuffer.length === 0) {
      throw new SystemError('圧縮対象の画像バッファが空です。', {
        statusCode: ERROR_STATUS_CODE.UNPROCESSABLE_ENTITY,
        severity: ERROR_SEVERITY.ERROR,
    ...
    try {
      ...
      if (this.activeFormat === 'avif') {
        compressedBuffer = await processed
          .avif({ quality: compressionQuality, effort: 4 })
          .toBuffer()
      } else {
      ...
      throw new SystemError('画像圧縮処理中にエラーが発生しました。', {
        statusCode: ERROR_STATUS_CODE.INTERNAL_SERVER_ERROR,
        severity: ERROR_SEVERITY.ERROR,
        callFunc: 'GoogleStorageService.compressImage',
  ...
  async uploadFileBuffer(
    ...
    if (buffer.length === 0) {
      throw new SystemError('アップロードするファイルデータが空です。', {
        statusCode: ERROR_STATUS_CODE.UNPROCESSABLE_ENTITY,
        severity: ERROR_SEVERITY.ERROR,
    ...
    try {
      ...
      // anyを許可してConvexエラーのチェックを行う
      if ((error as unknown) == typeof ConvexError) {
        throw error
      }
      throw new SystemError('バッファからのファイルアップロードに失敗しました。', {
        statusCode: ERROR_STATUS_CODE.INTERNAL_SERVER_ERROR,
        severity: ERROR_SEVERITY.ERROR,
        callFunc: 'GoogleStorageService.uploadFileBuffer',
  ...
  async uploadCompressedImageWithThumbnail(
    ...
    if (!base64Data) {
      ...
      throw new SystemError('処理するBase64画像データが提供されていません。', {
        statusCode: ERROR_STATUS_CODE.UNPROCESSABLE_ENTITY,
        severity: ERROR_SEVERITY.ERROR,
    ...
    try {
      ...
      throw new SystemError('Base64データのデコードに失敗しました。', {
        statusCode: ERROR_STATUS_CODE.UNPROCESSABLE_ENTITY,
        severity: ERROR_SEVERITY.ERROR,
        callFunc: 'GoogleStorageService.uploadCompressedImageWithThumbnail',
    ...
    if (imageBuffer.length === 0) {
      throw new SystemError('デコード後の画像データが空です。', {
        statusCode: ERROR_STATUS_CODE.UNPROCESSABLE_ENTITY,
        severity: ERROR_SEVERITY.ERROR,
        callFunc: 'GoogleStorageService.uploadCompressedImageWithThumbnail',
    ...
    const { originalQualityValue, originalWidth, thumbnailQualityValue, thumbnailWidth } =
      this.getCompressionSettings(quality)
    ...
    try {
      // 画像圧縮処理 (オリジナルとサムネイル)
      const originalCompressedBuffer = await this.compressImage(
        imageBuffer,
        originalWidth,
        aspectType,
        originalQualityValue
      )
      const thumbnailCompressedBuffer = await this.compressImage(
        imageBuffer,
        thumbnailWidth,
        aspectType,
        thumbnailQualityValue
      )

      if (originalCompressedBuffer.length === 0 || thumbnailCompressedBuffer.length === 0) {
        throw new SystemError('画像圧縮後、バッファが空になりました。', {
          statusCode: ERROR_STATUS_CODE.INTERNAL_SERVER_ERROR,
          severity: ERROR_SEVERITY.ERROR,
          callFunc: 'GoogleStorageService.uploadCompressedImageWithThumbnail',
      ...
      const baseName = sanitizeFileName(
        lastDotIndex === -1 ? safeOriginalFileName : safeOriginalFileName.substring(0, lastDotIndex)
      )

      // 通常: タイムスタンプ + サニタイズされたベース名 + 圧縮後の拡張子 (this.extension)
      const timestamp = new Date().toISOString().replace(/[-:Z]/g, '').split('.')[0]
      const finalOriginalName = `${timestamp}_${baseName}${this.extension}`
      const finalThumbnailName = `${timestamp}_${baseName}${this.extension}`
      ...
      if (uploadedOriginalFile?.filePath) {
        deletionPromises.push(
          this.deleteObjectByPath(uploadedOriginalFile.filePath).catch((delErr) => {
            console.error(
              'ロールバック：オリジナル画像の削除失敗:',
              uploadedOriginalFile?.filePath,
              this.formatErrorDetails(delErr)
            )
          })
        )
      }
      if (uploadedThumbnailFile?.filePath) {
        deletionPromises.push(
          this.deleteObjectByPath(uploadedThumbnailFile.filePath).catch((delErr) => {
            console.error(
              'ロールバック：サムネイル画像の削除失敗:',
              uploadedThumbnailFile?.filePath,
              this.formatErrorDetails(delErr)
            )
          })
        )
  ...
  private extractFilePath(imgUrl: string): string {
    if (!imgUrl || typeof imgUrl !== 'string') {
      throw new SystemError('imgUrlが有効な文字列ではありません。', {
        statusCode: ERROR_STATUS_CODE.UNPROCESSABLE_ENTITY,
        severity: ERROR_SEVERITY.ERROR,
    ...
    try {
      ...
      if (imgUrl.includes('://') || imgUrl.startsWith('/')) {
        throw new SystemError('imgUrlをファイルパスとして解析できませんでした。', {
          statusCode: ERROR_STATUS_CODE.UNPROCESSABLE_ENTITY,
          severity: ERROR_SEVERITY.ERROR,
          callFunc: 'GoogleStorageService.extractFilePath',
  ...
  async deleteImageWithThumbnail(imgUrl: string): Promise<void> {
    ...
    try {
      ...
      if (originalPath.includes('/original/')) {
        ...
        console.warn(
          `オリジナル画像のパスからサムネイルパスを推測できませんでした: ${originalPath}`
        )
      ...
      results.forEach((result, index) => {
        if (result.status === 'rejected') {
          ...
          console.warn(
            `${index === 0 ? 'オリジナル' : 'サムネイル'}画像の削除処理で問題が発生しました (詳細は先行ログ参照): ${path}`
          )
        }
      })
    } catch (error) {
  ...
  async getSignedUploadUrl(
    ...
    try {
      ...
      throw new SystemError('署名付きURLの生成に失敗しました', {
        statusCode: ERROR_STATUS_CODE.INTERNAL_SERVER_ERROR,
        severity: ERROR_SEVERITY.ERROR,
   ...
   */
  transformImageUrlsInObject<T extends Record<string, unknown>>(
    obj: T,

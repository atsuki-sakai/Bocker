...
export class ReservationRepository extends BaseRepository<'reservation'> {
  ...
  /**
   * SupabaseのIDを元に予約と予約詳細を取得します。
   * @param id - SupabaseのレコードID
   * @returns 予約情報と詳細情報またはnull
   */
  async findById(id: string): Promise<{
    reservation: RowType<'reservation'>
    reservationDetail: RowType<'reservation_detail'> | null
  } | null> {
    const { data } = await this.supabaseServiceInstance.listRecords('reservation', {
      filters: {
        uid: id,
      },
    })
    ...
    return {
      reservation,
      reservationDetail: details?.[0] || null,
    }
  }
  ...
  async findByCustomer(
    ...
    console.log(
      `[ReservationRepository] findByCustomer: tenantId=${tenantId}, orgId=${orgId}, customerUid=${customerUid}`
    )
  ...
  async findByCustomerWithDetails(
    ...
    console.log(
      `[ReservationRepository] findByCustomerWithDetails: tenantId=${tenantId}, orgId=${orgId}, customerUid=${customerUid}`
    )
    ...
    try {
      ...
      let rangeFilter:
        | { column: keyof RowType<'reservation'>; from?: string | number; to?: string | number }
        | undefined
      ...
      const { data: reservations, count } = await this.supabaseServiceInstance.listRecords(
        'reservation',
        {
          filters,
          rangeFilter,
      ...
      const formattedData = await Promise.all(
        (reservations || []).map(async (reservation) => {
          ...
          const { data: details } = await this.supabaseServiceInstance.listRecords(
            'reservation_detail',
            {
              filters: {
                _convex_reservation_id: reservation._convex_id,
  ...
  async findByDateRange(
    ...
    console.log(
      `[ReservationRepository] findByDateRange: tenantId=${tenantId}, orgId=${orgId}, start=${startDate}, end=${endDate}`
    )
  ...
  async findByStatus(
    ...
    console.log(
      `[ReservationRepository] findByStatus: tenantId=${tenantId}, orgId=${orgId}, status=${status}`
    )
  ...
  async getCustomerReservationStats(
    ...
    console.log(
      `[ReservationRepository] getCustomerReservationStats: tenantId=${tenantId}, orgId=${orgId}, customerUid=${customerUid}`
    )
    ...
    try {
      ...
      reservationDetails.forEach(({ reservation, detail }) => {
        ...
        if (reservation.status === 'completed') {
          ...
          Number(reservation.start_time_unix) > now
        ) {
          upcomingCount++
        }
      })
  ...
  async findByOrganizationWithDetails(
    ...
    console.log(
      `[ReservationRepository] findByOrganizationWithDetails: tenantId=${tenantId}, orgId=${orgId}`
    )
  ...
  async getOrganizationReservationStats(
    ...
    console.log(
      `[ReservationRepository] getOrganizationReservationStats: tenantId=${tenantId}, orgId=${orgId}`
    )
  ...
  async findByOrganizationWithDetailsOptimized(
    ...
    console.log(
      `[ReservationRepository] findByOrganizationWithDetailsOptimized: tenantId=${tenantId}, orgId=${orgId}`
    )
    ...
    try {
      ...
      // RPC関数を呼び出し
      const { data, error } =
        await this.supabaseServiceInstance.rpc<OptimizedReservationRpcResponse>(
  ...
  async exportAllReservationsOptimized(
    ...
    console.log(
      `[ReservationRepository] exportAllReservationsOptimized: tenantId=${tenantId}, orgId=${orgId}`
    )
    ...
    try {
      ...
      if (Array.isArray(reservations)) {
        formattedData = reservations.map(
          (item: {
            reservation: RowType<'reservation'>
            detail: RowType<'reservation_detail'> | null
          }) => ({
        ...
        formattedData = parsedReservations.map(
          (item: {
            reservation: RowType<'reservation'>
            detail: RowType<'reservation_detail'> | null
          }) => ({
      ...
      console.log(
        `[ReservationRepository] exportAllReservationsOptimized completed: ${formattedData.length} records in ${processingTimeMs}ms`
      )

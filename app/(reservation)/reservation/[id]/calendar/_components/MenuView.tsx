  ...
  return (
    <div className="w-full">
      ...
      >
        <TabsList className="flex flex-col h-full w-1/5  overflow-y-auto overflow-x-hidden  rounded-md overflow-hidden">
          {uniqueCategories.map((category) => (
            <TabsTrigger 
              key={category} 
              value={category}
              className="px-2 py-1 text-xs w-[80px] md:w-full text-wrap"
            >
              {category}
            </TabsTrigger>
          ))}
        ...
        {/* メニュー一覧 */}
        {uniqueCategories.map((category) => (
            <TabsContent key={category} value={category}  className='w-4/5 h-full max-h-[360px] md:max-h-[500px] overflow-y-auto' >
                        ...
                        {menu.tags && menu.tags.length > 0 && (
                          <div className="flex justify-end flex-wrap gap-0.5 scale-95">
                            {menu.tags.map((tag, idx) => (
                              <p key={idx} className="text-xs px-1 border border-green-700 text-green-700 rounded-full">
                                {tag}
                              </p>
                            ))}
                          </div>
                  ...
                  <CardContent className="p-2">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                       ...
                       <div className='flex items-center justify-between gap-2'>
                       <p className="text-xs scale-90 -ml-2 text-yellow-600 ">{convertPaymentMethod(menu.paymentMethod as PaymentMethod)}</p>
                       ...
                       >
                        ...
                        <div className="flex items-center justify-between gap-2 mt-1">
                          ...
                          <span className="text-sm text-gray-500">{menu.ensureTimeToMin}分</span>
                          </div>
                          {menu.salePrice ? (
                            <div className="flex items-center gap-1">
                              <span className="line-through text-xs text-gray-400">¥{menu.unitPrice?.toLocaleString()}</span>
                              <span className="font-bold text-green-600">¥{menu.salePrice.toLocaleString()}</span>
                            </div>
                          ) : (
        ...
        <div className="mt-4 border-t pt-2">
          ...
          <div className="space-y-2">
            {selectedMenus.map(menu => (
              <div key={menu._id} className="flex justify-between items-center p-1 bg-slate-50 rounded-md">
                <div>
                  <span className="text-xs text-gray-500">{menu.category || 'その他'} / {menu.ensureTimeToMin}分</span>
                  <span className="block text-sm font-bold">{menu.name}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className='text-green-600 font-bold'>¥{(menu.salePrice || menu.unitPrice)?.toLocaleString()}</span>
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    className="text-red-500 border h-8 w-8 p-0"
                    onClick={() => handleMenuSelect(menu)}
                  >
                    ×
                  </Button>
          ...
          {selectedMenu && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {selectedMenu.category || 'その他'} | {convertTarget(selectedMenu.targetType as Target)} | {convertGender(selectedMenu.targetGender as Gender)}
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <Clock className="h-4 w-4 text-gray-400" />
                    <span className="text-sm text-gray-500">{selectedMenu.ensureTimeToMin}分</span>
                  </div>
                  <div>
                    {selectedMenu.salePrice ? (
                      <div className="flex items-center gap-1">
                        <span className="line-through text-sm text-gray-400">¥{selectedMenu.unitPrice?.toLocaleString()}</span>
                        <span className="font-bold text-blue-600 text-lg">¥{selectedMenu.salePrice.toLocaleString()}</span>
                      </div>
                    ) : (
                      <span className="font-medium text-lg">¥{selectedMenu.unitPrice?.toLocaleString()}</span>
                    )}
                  </div>
                </div>
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {selectedMenu.tags.map((tag, idx) => (
                        <span key={idx} className="text-xs px-2 py-0.5 bg-green-50 border border-green-700 text-green-700 rounded-full">
                          {tag}
                        </span>
                      ))}
                    </div>
                ...
                <div className="mt-2">
                  ...
                  <p className="text-sm mt-1">{convertPaymentMethod(selectedMenu.paymentMethod as PaymentMethod)}</p>
                </div>
              </div>
              <DialogFooter>
                <Button 

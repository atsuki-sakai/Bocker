...
export default function EmailPreferencesPage() {
  ...
  // Clerkのメールアドレスをマッピング
  const emailAddresses: EmailAddress[] = user.emailAddresses.map(email => ({
    id: email.id,
  ...
  return (
    <DashboardSection title="メールアドレス設定" backLink="/dashboard" backLinkTitle="ダッシュボードに戻る">
        <p className="text-sm text-muted-foreground mb-4">
        メールアドレスを変更する場合は、<Link href="/dashboard/setting/change-email" className="text-blue-500 hover:underline">メールアドレス変更</Link>から可能です。
        </p>
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                <p className="my-2 font-bold text-sm">プライマリーに設定されたメールアドレスがログインとシステム通知に使用されます。</p>
                <br />
            ...
            <CardContent>
                ...
                <div className="space-y-4">
                    ...
                    >
                        ...
                        {!email.primary && (
                            <Dialog open={showDeleteDialog && emailToDelete === email.id} onOpenChange={(open) => {

...
if (description.includes("Unused time on")) {
    const planName = translatePlanName(description.replace("Unused time on ", "").split(" after")[0]);
    return `${planName}の未使用期間分の返金`;
}

if (description.includes("Remaining time on")) {
    const planName = translatePlanName(description.replace("Remaining time on ", "").split(" after")[0]);

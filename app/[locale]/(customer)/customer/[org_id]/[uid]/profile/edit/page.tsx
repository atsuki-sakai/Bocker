...
export default function ProfileEditPage({ params }: ProfileEditPageProps) {
  ...
  useEffect(() => {
    async function loadCustomerData() {
      ...
      try {
        ...
        if (customerData && customerData.customer) {
          setValue('firstName', customerData.customer.first_name || '')
          setValue('lastName', customerData.customer.last_name || '')
          setValue('email', customerData.customer.email || customerData.customerDetail?.email || '')
          setValue('phone', customerData.customer.phone || '')
          setValue('birthday', customerData.customerDetail?.birthday || '')
          const gender = customerData.customerDetail?.gender ?? 'unselected'
          setValue('gender', gender as Gender)
          setValue('notes', customerData.customerDetail?.notes || '')
        }
      } catch (error) {
    ...
    loadCustomerData()
  }, [session, uid, setValue, showErrorToast])
  ...
  const onSubmit = async (data: ProfileFormData) => {
    ...
    try {
      ...
      router.push(`/customer/${org_id}/${uid}/profile`)
    } catch (error) {
      showErrorToast(error)
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        <Link href={`/customer/${org_id}/${uid}/profile`}>

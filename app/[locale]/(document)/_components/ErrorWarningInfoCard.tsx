...
export default function ErrorWarningInfoCard({
  mainTitle = 'エラー・注意事項',
  errorItems,
  warningItems,
}: ErrorWarningInfoCardProps) {
  return (
    <Card>
      <CardHeader>
              ...
              <div className="space-y-3">
                {errorItems.map((item, index) => (
                  <div key={index} className="p-3 bg-destructive-foreground rounded-lg">
                    <p className="font-semibold text-destructive text-xs md:text-sm">
                      {item.title}
                    </p>
                    <p className="text-destructive text-xs md:text-sm mt-1">{item.description}</p>
                  </div>
                ))}
              </div>
            </div>
